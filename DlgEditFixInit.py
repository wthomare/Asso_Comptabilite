...
class DlgEditFixInit(wx.Dialog):
    
    def __init__(self, parent, ID, basicSoftwareFix):
        wx.Dialog.__init__(self, parent, ID, ("Init logAnalyzer from Prophet"), style=wx.RESIZE_BORDER|wx.CAPTION)
        ...
        self._unitTestInst = self._basicSoftwareFix.getUnitTestInst()
        
        lblUTI = wx.StaticText(self, wx.ID_ANY, ('Instance Name'))
        self._txtUTI = wx.TextCtrl(self, TXT_UTI, value=self._unitTestInst)
        ...
        self.Bind(wx.EVT_TEXT, self._onTxtUTIChange, id=TXT_UTI)
        ...
        szrParam.AddMany([(lblUTI, 0, wx.ALIGN_LEFT),
                          (self._txtUTI, 0, wx.EXPAND),
                          (lblDAF, 0, wx.ALIGN_LEFT),
    ...
    def _onCmdOk(self, event):
        self._basicSoftwareFix.setUnitTestInst(self._unitTestInst)
        ...
        _text = self._function + "(instance : " + str(self._unitTestInst) + ", format : " + str(self._dataFormat) + ", PTO : " + str(self._pollTimeout) + ', TMO : ' + str(self._timeout) + ')'
    ...
    def _onTxtUTIChange(self, event):
        self._unitTestInst=event.GetString().strip()
