...
from BasicSoftwareConst import expected

import wx

[TXT_RELATIONSHIP, BTN_REMOVE, TXT_PRICE, TXT_HC, TXT_QTY] = range(5)


class DlgEditState(wx.Dialog):
    def _init__(self, parent, ID, basicSoftwareLink):
        
        wx.Dialog.__init__(self, parent, ID, ("Expect answer to an order"), style=wx.RESIZE_BORDER|wx.CAPTION)
        
        self._basicSoftwareLink = basicSoftwareLink
        self._relationship = self._basicSoftwareLink.getRelationship()
        self._expected_selection_state = self._basicSoftwareLink.getExpectedAnswer()
        self._returnAction = wx.ID_CANCEL
        self.ExpectedBehavior = expected
        ...
        self.lb = wx.ComboBox(self, wx.ID_ANY, size=(200,-1), choices=self.ExpectedBehavior, style=wx.CB_DROPDOWN, value=self._expected_selection_state)
        ...
        szr1.AddMany([(lblRela, 0, wx.ALIGN_CENTER_HORIZONTAL),
                      ...
                      (line, 0, wx.GROW|wx.ALIGN_CENTER_HORIZONTAL|wx.TOP|wx.RIGHT)])
    ...
    def _onTextRelationshipChange(self, event):
        self._relationship = event.GetString()
        
    def _onCmdOk(self, event):
        self._basicSoftwareLink.setName("Expected " + self._expected_selection_state + ' for order :' +self._relationship)
        self._basicSoftwareLink.setExpectedAnswer(self._expected_selection_state)
